...
# The box we want to remove is at the bottom, marked by {/* Box: Relatório de Recebimentos */} and ending with </div> </div> </div> </div>... wait, we can just use regex to remove it.
import re
box_pattern = r'\{\/\* Box: Relatório de Recebimentos \*\/\}.*?Arraste o relatório de Recebimentos aqui<\/p>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>'
content = re.sub(box_pattern, '', content, flags=re.DOTALL)

# Now, we want to insert it after the importSuccess block.
target_str = """                </div>
              )}
            </div>"""
